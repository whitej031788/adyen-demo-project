...
let paymentMethodsDataObj = {
    ...
    "blockedPaymentMethods":["wechatpay","amazonpay"],
    "additionalData": {
...
let paymentDataObj = {
    ...
    "returnUrl":"https://localhost:3000",
      ...
      "shopperStatement":{
        "shopperStatement":"Statement Descriptor",
        "brandName":"PETER"
      },
      "metadata":{
        "key":"value"
      ...
      // "PayPal":{
      //   "CountryCode":"GB",
      //   "EmailId":"luke.strudwick@adyen.com",
      //   "FirstName":"FirstName",
      //   "LastName":"LastName",
      //   "PayerId":"1234abcd",
      //   "Phone":"123456789",
...
let newPbl = new PayByLink(paymentMethodsDataObj);
...
const translations = {
  "en-GB": {
   ...
   //"payButton":"Pay Unite Students",
   "paymentMethods.alipay_wap":"alipay mobile"
}
};
// Wrap all of this in a function we we can easily call payment methods again for country change
function getPaymentMethods() {
    checkoutApi.getPaymentMethods(paymentMethodsDataObj, window.demoSession.allowedPaymentMethods).then(async function (paymentMethodsResponse) {
        let configuration = {
...
function countryChange() {
  ...
  let countryToLocaleMap = {
    ...
    paymentMethodsDataObj.countryCode = countryCode;
    paymentMethodsDataObj.amount.currency = currencyCode;
    paymentMethodsDataObj.shopperLocale = locale;
    paymentDataObj.countryCode = countryCode;
    paymentDataObj.amount.currency = currencyCode;
    paymentDataObj.shopperLocale = locale;

    newPbl = new PayByLink(paymentMethodsDataObj);
    terminalApi = new TerminalApi(paymentDataObj);
    checkoutApi = new CheckoutApi(paymentDataObj);
    getPaymentMethods();
}
